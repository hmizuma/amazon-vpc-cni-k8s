...
func (ds *DataStore) AddENI(eniID string, deviceNumber int, isPrimary bool, mac string) error {
	ds.lock.Lock()
...
func (ds *DataStore) AssignPodIPv4Address(k8sPod *k8sapi.K8SPodInfo) (string, int, string, error) {
	ds.lock.Lock()
	defer ds.lock.Unlock()
	...
	if ok {
		if ipAddr.IP == k8sPod.IP && k8sPod.IP != "" {
			...
			return ipAddr.IP, ipAddr.DeviceNumber, ipAddr.MAC, nil
		}
...
// It returns the assigned IPv4 address, device number, error
func (ds *DataStore) assignPodIPv4AddressUnsafe(k8sPod *k8sapi.K8SPodInfo) (string, int, string, error) {
	podKey := PodKey{
		name:      k8sPod.Name,
		namespace: k8sPod.Namespace,
		container: k8sPod.Container,
	...
	for _, eni := range ds.eniIPPools {
		...
		for _, addr := range eni.IPv4Addresses {
			if k8sPod.IP == addr.address {
				...
				ds.podsIP[podKey] = PodIPInfo{IP: addr.address, DeviceNumber: eni.DeviceNumber, MAC: eni.MAC}
				log.Infof("AssignPodIPv4Address Reassign IP %v to pod (name %s, namespace %s)",
					addr.address, k8sPod.Name, k8sPod.Namespace)
				return addr.address, eni.DeviceNumber, eni.MAC, nil
			}
