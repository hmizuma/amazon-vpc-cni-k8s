...
func (s *server) AddNetwork(ctx context.Context, in *pb.AddNetworkRequest) (*pb.AddNetworkReply, error) {
	...
	addr, deviceNumber, mac, err := s.ipamContext.dataStore.AssignPodIPv4Address(&k8sapi.K8SPodInfo{
		Name:      in.K8S_POD_NAME,
		Namespace: in.K8S_POD_NAMESPACE,
		Container: in.K8S_POD_INFRA_CONTAINER_ID})
	log.Infof("Send AddNetworkReply: IPv4Addr %s, DeviceNumber: %d, MAC: %s, err: %v", addr, deviceNumber, mac, err)
	addIPCnt.Inc()
	return &pb.AddNetworkReply{Success: err == nil, IPv4Addr: addr, IPv4Subnet: "", DeviceNumber: int32(deviceNumber), MAC: mac}, nil
}
